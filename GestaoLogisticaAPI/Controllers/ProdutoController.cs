...
[Authorize]
[Route("produtos")]
public class ProdutoController(IProdutoService service) : AbstractController
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] ProdutoDto dto)
    {
        ...
        return CreatedAtAction(nameof(Get), new { id = created.CodProduto }, created);
    }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] ProdutoDto dto)
    {
        ...
        if (id != dto.CodProduto)
            return BadRequest(new { message = "Id do produto diferente do id no body." });
