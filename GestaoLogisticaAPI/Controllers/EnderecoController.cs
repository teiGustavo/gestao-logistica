...
[Authorize]
[Route("enderecos")]
public class EnderecoController(IEnderecoService service) : AbstractController
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] EnderecoDto dto)
    {
        ...
        return CreatedAtAction(nameof(Get), new { id = created.CodEndereco }, created);
    }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] EnderecoDto dto)
    {
        ...
        if (id != dto.CodEndereco)
            return BadRequest(new { message = "Id do endereço diferente do id no body." });
