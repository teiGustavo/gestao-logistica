...
[Authorize]
[Route("estoques")]
public class EstoqueController(IEstoqueService service) : AbstractController
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] EstoqueDto dto)
    {
        ...
        dto.CodEstoque = 0;
        dto.AtualizadoEm = null;

        var result = await service.CreateAsync(dto);
        if (!result.IsSuccess) return BadRequest(new { error = result.ErrorMessage });

        var created = result.Value!;
        return CreatedAtAction(nameof(Get), new { id = created.CodEstoque }, created);
    }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] EstoqueDto dto)
    {
        ...
        if (id != dto.CodEstoque)
            return BadRequest(new { message = "Id do estoque diferente do id no body." });
