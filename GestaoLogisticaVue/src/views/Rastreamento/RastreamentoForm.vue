...
onMounted(async () => {
  ...
  const entRes = await entregaService.list();
  entregas.value = entRes.map((x: any) => ({
  ...
  // EDITAR
  if (id) {
    const d = await rastreamentoService.get(Number(id));
    if (d) {
...
async function save() {
  ...
  if (id) await rastreamentoService.update(Number(id), payload);
  else await rastreamentoService.create(payload);
        ...
        <v-card-text>
          <v-form @submit.prevent="save">
                ...
                >
                  <template #prepend-item>
                    <v-btn
                      ...
                      color="primary"
                      block
                      @click.stop="router.push({ name: 'entrega-create' })"
                    >
                      Nova Entrega
                    </v-btn>
            ...
            <v-btn
              color="secondary"
              class="mt-4 ml-2"
              @click="$router.push({ name: 'rastreamento' })"
            >
              Cancelar
            </v-btn>
          </v-form>
